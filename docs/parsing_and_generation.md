...
- The first block written is the base loader code, generated by `get_base_file_loader_code()` from `src/code_helper/mod.rs`.
    ...
    - Environment variables (API URL, username, password, base URL) are injected at runtime using Rust's `std::env::var` or replaced with defaults.
...
- **Environment Variables:** Authentication and API URLs are injected via environment variables or user prompts for flexibility.
